...
* 이 모든 것을 확인해야 합니다 link:prerequisites-azure-connector-snapcenter-service.html["필수 구성 요소"] 충족됩니다.
...
. 있는 경우 link:prerequisites-azure-connector-snapcenter-service.html#firewall-configuration["방화벽이 구성되었습니다"]에서 를 사용하여 사용자 지정 역할을 만듭니다 link:media/SnapCenter-Deployment-Role3.json["SnapCenter - 배포 - Role3.json"] 파일.
+
